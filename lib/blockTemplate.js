...
var merkleroot = require('./merkleTree');
...
var BlockTemplate = module.exports = function BlockTemplate(jobId, rpcData, poolAddressScript, extraNoncePlaceholder, reward, txMessages, recipients) {
    ...
    this.target = rpcData.target ?
            new BN(rpcData.target, 'hex', 'be') :
            util.bignumFromBitsHex(rpcData.bits);
    
    var limit = bits2targetBN(getMaxTarget());
    var t = limit.div(this.target).toString(10);
    this.difficulty = parseFloat(t).toFixed(9);
    ...
    this.merkleTree = new merkleroot(getTransactionBuffers(rpcData.transactions));
