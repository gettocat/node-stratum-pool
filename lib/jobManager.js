...
var JobManager = module.exports = function JobManager(options){
    ...
    this.processShare = function(jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName){
        ...
        var headerBigNum = new BN(headerHash.toString('hex'), 'hex', 'le');
        var blockHashInvalid;
        var blockHash;
        var blockHex;


        var shareDiff = bits2targetBN(getMaxTarget()).div(headerBigNum).mul(new BN(shareMultiplier));
        var blockDiffAdjusted = job.difficulty * shareMultiplier;

        //Check if share is a block candidate (matched network difficulty)
        if (job.target.gt(headerBigNum)){
        ...
        _this.emit('share', {
            ...
            difficulty: difficulty,
            shareDiff: parseFloat(shareDiff.toString(10)).toFixed(8),
