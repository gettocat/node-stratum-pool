...
var dbconnection = require('./db/connection')(__dirname + "/../")
...
var pool = module.exports = function pool(options, authorizeFn) {
    ...
    function SetupJobManager() {
        ...
        _this.jobManager.on('newBlock', function (blockTemplate) {
            ...
            var emitShare = function () {
                console.log('emit.daemon:', _this.daemon);
                _this.emit('share', isValidShare, isValidBlock, shareData, _this.daemon);
            };
    ...
    function DetectCoinData(finishedCallback) {
        ...
        _this.daemon.batchCmd(batchRpcCalls, function (error, results, data) {
            if (error || !results) {
    ...
    this.processBlockNotify = function (blockHash, sourceTrigger) {
        emitLog('Block notification ' + blockHash + ' via ' + sourceTrigger);
